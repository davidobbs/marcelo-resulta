...
export default function DREPage() {
  ...
  const { analysisPeriod } = useAppStore();
    ...
    <div className="p-6 max-w-7xl mx-auto">
      ...
      <div className="card mb-6">
        <div className="card-header">
          <h2 className="text-xl font-semibold">Demonstração do Resultado ({getPeriodLabel()})</h2>
        </div>
          ...
          <div className="overflow-x-auto">
            <table className="table">
              ...
              <tbody>
                ...
                {Object.entries(receitas).map(([key, value]) => (
                ...
                <tr className="table-row bg-red-50 dark:bg-red-900/20">
                  <td className="table-cell font-semibold text-red-600 dark:text-red-400">CUSTOS OPERACIONAIS</td>
                  <td colSpan={2}></td>
                </tr>
                {Object.entries(custos).map(([key, value]) => (
                   <tr key={key} className="table-row">
                   <td className="table-cell pl-8">{accountLabels[key] || key}</td>
                ...
                {/* EBITDA */}
                <tr className="table-row bg-purple-50 dark:bg-purple-900/20">
                  <td className="table-cell font-semibold text-purple-700 dark:text-purple-300">EBITDA</td>
                  <td className="table-cell text-right font-bold text-purple-600">
                    {formatCurrency(dre?.ebitda || 0)}
                  </td>
                  <td className="table-cell text-right font-semibold text-purple-600">
                    {formatPercentage(dre?.margemEbitda || 0)}
                  </td>
                </tr>
                ...
                <tr className="table-row">
                  ...
                  <td className="table-cell text-right text-gray-600 font-medium">
                    ({formatCurrency(dre?.impostos || 0)})
                  </td>
                  <td className="table-cell text-right text-sm text-gray-600">
                    {formatPercentage((dre?.impostos || 0) / totalRevenue * 100)}
                  </td>
                </tr>
                <tr className="table-row bg-gray-900 dark:bg-gray-100">
                  ...
                  <td className="table-cell text-right font-bold text-white dark:text-gray-900">
                    {formatPercentage(dre?.margemLiquida || 0)}
                  </td>
                </tr>
              </tbody>
            </table>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
        <div className="metric-card">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="metric-value">{formatPercentage(dre?.margemBruta || 0)}</p>
            </div>
        ...
        <div className="metric-card">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="metric-value">{formatPercentage(dre?.margemEbitda || 0)}</p>
            </div>
        ...
        <div className="metric-card">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="metric-value">{formatPercentage(dre?.margemLiquida || 0)}</p>
            </div>
        ...
        <div className="metric-card">
          <div className="flex items-center justify-between">
            <div>
              <p className="metric-label">ROI (sobre Receita)</p>
              <p className="metric-value">{formatPercentage((dre?.lucroLiquido || 0) / totalRevenue * 100)}</p>
            </div>
              ...
              <div className="space-y-3">
                ...
                {(dre?.lucroLiquido || 0) > 0 && (
                  <div className="flex items-start gap-2">
                    <CheckCircle className="h-5 w-5 text-green-600 mt-0.5" />
                    <p className="text-sm text-gray-700 dark:text-gray-300">
                      Resultado líquido positivo no período.
                    </p>
                  </div>
                )}
                {(dre?.margemEbitda || 0) > 20 && (
                  <div className="flex items-start gap-2">
                    <CheckCircle className="h-5 w-5 text-green-600 mt-0.5" />
                    <p className="text-sm text-gray-700 dark:text-gray-300">
                      EBITDA com margem superior a 20%, indicando boa eficiência operacional.
                    </p>
                  </div>
                )}
              </div>
              ...
              <div className="space-y-3">
                ...
                {(dre?.lucroLiquido || 0) < 0 && (
                  <div className="flex items-start gap-2">
                    ...
                    <p className="text-sm text-gray-700 dark:text-gray-300">
                      Prejuízo no período - necessário revisar estratégia de precificação ou estrutura de custos.
                    </p>
                  </div>
                )}
